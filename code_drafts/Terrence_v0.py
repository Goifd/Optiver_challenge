    ...
    if trade_volume != 0:
        if stock_position < total_delta:
            if abs(stock_position) < total_delta:
                ...
                if not trade_would_breach_position_limit(stock_id, 100, 'ask'):
                    print("66111")
                    exchange.insert_order(stock_id, price = order_book1.bids[0].price, volume=trade_volume, side='ask', order_type='ioc')
            else:
                print("1112222111")
                print(order_book1.asks[0].price)
                if not trade_would_breach_position_limit(stock_id, 100, 'ask'):
                    exchange.insert_order(stock_id, price = order_book1.asks[0].price, volume=trade_volume, side='bid', order_type='ioc')
